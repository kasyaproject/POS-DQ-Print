<x-app-layout>
    @section('sidebar#pengguna','bg-gray-100 dark:bg-gray-700')    
...
@section('container');
            ...
            <div class="absolute z-10 max-xl:w-full xl:w-[83%] bg-gray-">
                <div class="flex justify-center lg:mx-72 bg-blue-">
                    <div id="alert-1" class="flex items-center p-4 mb-4 text-red-800 rounded-lg bg-red-50 dark:bg-red-800 dark:text-red-400" role="alert">
                        ...
                        Data gagal ditambahkan, silakan perbaiki data pengguna baru.
                        </div>
    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-4">
        <div class="flex items-center justify-between my-2 sm:mx-8 max-sm:px-2">            
            <form action="{{ route('pengguna.index') }}" method="GET" class="max-sm:w-60 sm:w-96">  
                @csrf 
        ...
        <div id="static-modal" data-modal-backdrop="static" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
            <div class="relative p-4 w-full max-w-xl max-h-full">
                ...
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <!-- Modal body -->
                    <form action="{{ route('pengguna.store') }}" method="POST" class="p-4 md:p-5">
                        @csrf
                        <div class="grid gap-4 mb-4 grid-cols-2">
                            <div class="col-span-2">
                                <label for="nama" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama pengguna</label>
                                <input type="text" name="nama" id="nama" value="{{ old('nama') }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Masukan nama pengguna" required="" oninput="this.value = this.value.replace(/[^A-Za-z ]/g, '');">
                                @error('nama')
                            ...
                            <div class="col-span-2 sm:col-span-1">
                                <label for="nik" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nik Pengguna</label>
                                <input type="text" name="nik" id="nik" value="{{ old('nik') }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Masukan nik pengguna" required="" oninput="this.value = this.value.replace(/[^0-9]/g, '');">
                                @error('nik')
                                    <p id="filled_error_help" class="mt-2 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            </div>
                            <div class="col-span-2 sm:col-span-1">
                                ...
                                <select id="jabatan" name="jabatan" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                                    <option selected="">Pilih Jabatan pengguna</option>
                                    <option value="Admin" {{ old('jabatan') == 'Admin' ? 'selected' : '' }}>Admin</option>
                                    <option value="Keuangan" {{ old('jabatan') == 'Keuangan' ? 'selected' : '' }}>Staff Keuangan</option>   
                                    <option value="Kasir" {{ old('jabatan') == 'Kasir' ? 'selected' : '' }}>Staff Kasir</option>
                                    <option value="Design" {{ old('jabatan') == 'Design' ? 'selected' : '' }}>Staff Design</option>
                                    <option value="Operator A3" {{ old('jabatan') == 'Operator A3' ? 'selected' : '' }}>Staff Operator A3</option>
                                    <option value="Operator OD/ID" {{ old('jabatan') == 'Operator OD/ID' ? 'selected' : '' }}>Staff Operator OD/ID</option>
                                    <option value="Finishing" {{ old('jabatan') == 'Finishing' ? 'selected' : '' }}>Staff Finishing</option>
                                </select>
                                @error('jabatan')
                                    <p id="filled_error_help" class="mt-2 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
        ...
        @foreach ($pengguna as $data)
            <div id="default-modal-{{ $data->id }}" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                <div class="relative p-4 w-full max-w-2xl max-h-full">
                    <!-- Modal content -->
                    <form action="{{ route('pengguna.update', ['pengguna' => $data->id]) }}" method="POST" class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                        @method('PUT')
                        @csrf
                        <!-- Modal header -->
                        <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                        ...
                        <div class="p-4 md:p-5 space-y-4">
                            <div class="col-span-2">
                                <label for="nama" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama pengguna</label>
                                <input type="text" name="nama" id="nama" value="{{ $data->nama }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Masukan nama pengguna" required="" oninput="this.value = this.value.replace(/[^A-Za-z ]/g, '');">
                                @error('nama')
                                    <p id="filled_error_help" class="mt-2 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            </div>
                            <div class="col-span-2">
                                <label for="email" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Email Pengguna</label>
                                <input type="email" name="email" id="email" value="{{ $data->email }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Masukan email pengguna" required="">
                                @error('email')
                                    <p id="filled_error_help" class="mt-2 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            </div>
                            <div class="col-span-2 sm:col-span-1">
                                <label for="nik" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nik Pengguna</label>
                                <input type="text" name="nik" id="nik" value="{{ $data->nik }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Masukan nik pengguna" required="" oninput="this.value = this.value.replace(/[^0-9]/g, '');">
                                @error('nik')
                                    <p id="filled_error_help" class="mt-2 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            </div>
                            <div class="col-span-2 sm:col-span-1">
                                ...
                                <select id="jabatan" name="jabatan" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                                    ...
                                    <option value="Keuangan" {{ $data->jabatan == 'Keuangan' ? 'selected' : '' }}>Staff Keuangan</option>   
                                    <option value="Kasir" {{ $data->jabatan == 'Kasir' ? 'selected' : '' }}>Staff Kasir</option>
                                    <option value="Design" {{ $data->jabatan == 'Design' ? 'selected' : '' }}>Staff Design</option>
                                    <option value="Operator A3" {{ $data->jabatan == 'Operator A3' ? 'selected' : '' }}>Staff Operator A3</option>
                                    <option value="Operator OD/ID" {{ $data->jabatan == 'Operator OD/ID' ? 'selected' : '' }}>Staff Operator OD/ID</option>
                                    <option value="Finishing" {{ $data->jabatan == 'Finishing' ? 'selected' : '' }}>Staff Finishing</option>
                                </select>
                                @error('jabatan')
                                    <p id="filled_error_help" class="mt-2 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
        ...
        @foreach ($pengguna as $data)
            <div id="popup-modal-{{ $data->id }}" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                <div class="relative p-4 w-full max-w-md max-h-full">
                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                        <form action="{{ route('pengguna.destroy', ['pengguna' => $data->id]) }}" method="POST" class="p-4 md:p-5 text-center">
                            @method('DELETE')
                            @csrf
                            ...
                            <h3 class="mb-10 text-lg font-semibold text-black dark:text-white">{{ $data->nama }} - {{ $data->nik }}</h3>
